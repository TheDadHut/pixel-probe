...
| `exif_rich.jpg` | 2 | Generated (`scripts/build_fixtures.py` via `piexif`) | 100×100 JPEG with Make/Model + Exif sub-IFD (ExposureTime, FNumber, ISOSpeedRatings, FocalLength, DateTimeOriginal) plus a deliberately oversized 100-byte MakerNote that exercises the bytes-summarization gate | _SHA varies with Pillow encoder; not asserted_ |
| `exif_with_gps.jpg` | 2 | Generated (`scripts/build_fixtures.py` via `piexif`) | 100×100 JPEG with GPS sub-IFD: 37° 46' 30" N, 122° 25' 15" W | _SHA varies with Pillow encoder; not asserted_ |
